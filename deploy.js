...
(async () => {
    try {
        console.log('Getting deployment data')
        ...
        const {data: artifacts} = await requestWithAuth('GET /repos/{owner}/{repo}/actions/runs/{run_id}/artifacts', {run_id});
        const artifact = artifacts.artifacts.find(x => x.name === 'dist');
